...
def test_game_can_load_events() -> None:
    ...
    historic_events = [domain_events.GameStarted("id-1", "player-1", "player-2")]
...
def test_winning_move_results_in_game_finished_event() -> None:
    ...
    historic_events = [
        domain_events.GameStarted("id-1", "p1", "p2"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="A"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="A"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="B"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="B"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="C"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="C"),
    ]
    game_obj.load_from_history(historic_events)
    # AND a move that will win the game for player 1
    move = game.Move(player="p1", column="D", turn=7)

    # WHEN the move is made
    game_obj.make_move(move)

    # THEN a move event is created for the move
    move_made = events.MoveMade(game_id=game_obj.id, player_id="p1", column="D")
...
def test_non_winning_move_that_fills_the_board_results_in_game_tied_event() -> None:
    ...
    historic_events = [
        ...
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="B"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="C"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="D"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="C"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="C"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="C"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="C"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="C"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="F"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="D"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="D"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="E"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="D"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="D"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="E"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="D"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="F"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="E"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="E"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="E"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="E"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="F"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="G"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="G"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="G"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="G"),
        events.MoveMade(game_id=game_obj.id, player_id="p2", column="F"),
        events.MoveMade(game_id=game_obj.id, player_id="p1", column="G"),
    ...
    game_obj.load_from_history(historic_events)
    # AND a move that will win the game for player 1
    move = game.Move(player="p2", column="F", turn=42)

    # WHEN the move is made
    game_obj.make_move(move)

    # THEN a move event is created for the move
    move_made = events.MoveMade(game_id=game_obj.id, player_id="p2", column="F")
    assert move_made in game_obj.uncommitted_events
    # AND a GameFinished event indicating player 1 has won is created
...
def test_move_that_does_not_finish_game_does_not_emit_finish_related_event() -> None:
    ...
    move = game.Move(player="p1", column="G", turn=3)

    # WHEN the move is made
    game_obj.make_move(move)

    # THEN the move made event is created
    mode_made = domain_events.MoveMade(game_id=game_obj.id, player_id="p1", column="G")
    assert mode_made in game_obj.uncommitted_events
    # BUT no GameEnded event is created
    assert not any(
        isinstance(event, domain_events.GameEnded)
        for event in game_obj.uncommitted_events
    )
